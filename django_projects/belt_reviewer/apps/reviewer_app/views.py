...
def registration(request):
    ...
    if len(first_name) < 3:
        ...
        messages.add_message(request, messages.ERROR,
                             "*First Name has to be at least 3 characters!")
    if first_name.isalpha == False:
        form_reproved = True
        messages.add_message(request, messages.ERROR,
                             "*First Name can only hold alphabetic characters!")
    if len(last_name) < 3:
        form_reproved = True
        messages.add_message(request, messages.ERROR,
                             "*Last Name has to be at least 3 characters!")
    if last_name.isalpha == False:
        form_reproved = True
        messages.add_message(request, messages.ERROR,
                             "*Last Name can only hold alphabetic characters!")
    if len(password) < 4:
        messages.add_message(request, messages.ERROR,
    ...
    if password.isnumeric() == True or password.isalpha() == True or password.islower() == True or password.isupper() == True:
        messages.add_message(
            request, messages.ERROR, '*Password is requred to have at least 1 uppercase, 1 lowercase letter and 1 numeric value!')
        form_reproved = True
    ...
    if form_reproved == True:
        ...
        if register_user.select_one_user_by_email(email) is not False:
            messages.add_message(
                request, messages.ERROR, '*This e-mail belongs to a registered user! - Either Log In or register with another e-mail')
            return redirect('/')
        else:
            register_user.add_user(first_name, last_name, email, password)
            request.session['first_name'] = register_user.get_last_User(
            ).first_name
...
def login(request):
    ...
    if len(email) < 1:
        messages.add_message(request, messages.ERROR,
                             "*You have to provide an user name!")
        is_valid = False
    if len(password) < 1:
        messages.add_message(request, messages.ERROR,
    ...
    if is_valid == False:
        ...
        if user.select_one_user_by_email(email) is False:
            messages.add_message(request, messages.ERROR,
                                 "*Username (e-mail) does not exist")
            return redirect('/')
        else:
            username.select_one_user_by_email(email)
            if user.password != password:
                messages.add_message(
                    request, messages.ERROR, "*Password does not match user")
                return redirect('/')
            else:
...
def book_process(request):
    ...
    if len(title) < 1:
        isvalid = False
        messages.add_message(request, messages.ERROR,
                             "*Please include a book title")
    if book.select_one_book_by_name(title) is True:
        isvalid = False
        messages.add_message(request, messages.ERROR,
                             "*A book with the same name has already been added")
    if len(author) < 1 and author_select is False:
        isvalid = False
        messages.add_message(request, messages.ERROR,
                             "*Please choose one author")
    if len(author) > 1 and author_select is not False:
        isvalid = False
        messages.add_message(request, messages.ERROR,
                             "*Please choose just one author in one field")
    if len(review) < 1:
        isvalid = False
        messages.add_message(request, messages.ERROR,
    ...
    if isvalid == False:
        ...
        review_class.add_review(review, rating, user.select_one_user_by_email(
            email_input), book.get_last_book())
...
def add_review_process(request):
    ...
    if is_valid == False:
        ...
        review_class.add_review(review, new_rating, user.select_one_user_by_email(
            email_input), book.select_one_book(book_id))
        return redirect('/books/' + str(book_id))
