    ...
    if len(request.POST['new_word']) > 1:
        ...
        if 'results' not in request.session:
            ...
            resultslist = [{'word': html_new_word,
                            'color': html_color}]
            request.session['results']['listdicts']['word'].append(
            '{}    - added on {}'.format(html_new_word, strftime("%b %d, %Y %H:%M %p", gmtime())))
            request.session.modified = True
            request.session['results']['listdicts']['color'].append(
                '{}    - added on {}'.format(html_color, strftime("%b %d, %Y %H:%M %p", gmtime())))
            request.session.modified = True
